...
ker_sz = 3;
kernel = ones(ker_sz,ker_sz) ./ (ker_sz^2); %Convolution kernel
disp(kernel);

% Function
spatial_avg = @(mx) conv2(mx, kernel, 'same'); %NB!! makes a small mistake near boundaries


ex1 = conv2(mx, ones(3,3), 'same'); %Does not divide
disp(ex1);
...
%{
fun = @(matrix) mean2(matrix.data);
smart_avg = blockproc(mx, [ker_sz ker_sz], fun);

%smart_avg = BlockMean(mx, ker_sz, ker_sz);
%smart_avg = kron(smart_avg, ones(ker_sz, ker_sz));
