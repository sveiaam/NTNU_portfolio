...
#   Funksjon for å estimere usikkerhet i tilpasningsparametre
def usikkerhet_bootstrap(S,t,x0,N):
    ...
    phi=zeros(N)
    for i in range(N):
        index=randint(0,len(S),len(S))  # Tabell med et tilfeldig(trukket med tilbakelegging) utvalg av tall mellom0 og antall datapunketer
    ...
    print('Middelverdier:\n<S0>=\t%e mm\n<A>=\t%e mm\n<Alpha>=%e 1/s\n<w>=\t%e (rad)/s\n<T>=\t%e s\n<phi>=\t%e (rad)' % (mean(S0),mean(A),mean(Alpha),mean(w),mean(T),mean(phi)))
    dx=(std(S0,ddof=1+nParams),std(A,ddof=1+nParams),std(Alpha,ddof=1+nParams),std(w,ddof=1+nParams),std(phi,ddof=1+nParams))
    print('Usikkerheter:\n<dS0>=\t%e mm\n<dA>=\t%e mm\n<dAlpha>=%e 1/s\n<dw>=\t%e (rad)/s\n<dphi>=\t%e (rad)' % dx)
...
if __name__=='__main__':
    data=loadtxt('Måling_1.txt')  # Lese data fra fil
    t=data[:,0]*60+data[:,1]            # Tid: minutter i første, og sekunder i andre kolonne
    S=data[:,2]                         # Utsving i tredje kolonne

    # Initielle verdier for tilpasningen
    S0_0=660    # Likevektslinje
    ...
    plot(t_plot,S0+A*exp(-Alpha*t_plot),'--k')  # Øvre omhyldningskurve
