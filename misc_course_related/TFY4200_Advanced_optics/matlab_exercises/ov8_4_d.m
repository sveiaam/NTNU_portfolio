...
for j = 1:l
    ...
    Mmid = [ Ai(n_1(j), k_1(j), 2*d1), Bi(n_1(j), k_1(j), 2*d1) ;
           Ci(n_1(j), k_1(j), 2*d1), Di(n_1(j), k_1(j), 2*d1) ];   
...
legend({'Fabry-Perot'}, 'Location', 'northeast');
xt = get(gca, 'XTick');
